...
public class SpawnWork implements Runnable {
    ...
    public void FixEntity(LivingEntity livingEntity) {
        ...
        if(isPigmanType(livingEntity.getType())) PigMan(livingEntity);
        if(livingEntity instanceof Zombie) Zombie((Zombie)livingEntity);
    }

    private boolean isPigmanType(EntityType entityType) {
        return entityType.name().equals("PIG_ZOMBIE") || entityType.name().equals("ZOMBIFIED_PIGLIN");
    }
    ...
    public void PigMan(LivingEntity pigzombie) {
        pigzombie.getEquipment().setItemInMainHand(new ItemStack(Material.GOLDEN_SWORD));
        if(!plugin.pigZombieConfig.Aggressive) {
            return;
        }
        try {
            pigzombie.getClass().getMethod("setAngry", boolean.class).invoke(pigzombie, true);
        } catch (Exception ignored) {
            // The server implementation may not expose anger control on this entity type.
        }
    }
